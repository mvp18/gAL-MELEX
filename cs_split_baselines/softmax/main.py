...
parser.add_argument('-data', '--dataset', help='choose between APY/AWA2', default='AwA', type=str)
...
if args.dataset=='aPY': 
	...
	cls_splits = np.load('../../'+args.dataset+'/'+'apy_cs_split.npy', allow_pickle=True).item()
	...
	cls_splits = np.load('../../'+args.dataset+'/'+'awa_cs_split.npy', allow_pickle=True).item()
...
# Shape -> (dxN)
X_train = feat[:, train_loc].T
X_val = feat[:, val_loc].T
X_test = feat[:, test_loc].T

print('Tr:{}; Val:{}; Ts:{}\n'.format(X_train.shape[0], X_val.shape[0], X_test.shape[0]))

labels = res101['labels']
labels_train = np.squeeze(labels[train_loc])
labels_val = np.squeeze(labels[val_loc])
labels_test = np.squeeze(labels[test_loc])
