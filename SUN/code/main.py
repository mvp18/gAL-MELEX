...
def main(args):
	...
	predicates = [x[0][0] for x in io.loadmat('../attributes.mat')['attributes']]

	if args.data_split == 'gbu':
		print('\nLoading data with GBU splits')
		img_name_list, class_list, signature_list, name2feat, name2class, name2att = load_data()
		ds_type = '' # This is used later while createing folder names
	elif args.data_split == 'cs':
		new_split_file = '../sun_cs_split.npy'
		print('\Loading data with new splits as in ' + new_split_file)
		img_name_list, class_list, signature_list, name2feat, name2class, name2att = load_data_custom_split(new_split_file)
		ds_type = '_corr_shift'
	...
	spectral_groups, attr_distance = create_spectral_groups(num_clusters=args.clusters, random_state=random_state, predicates=predicates,
															train_ids=img_name_list[0], test_prior=signature_list[2], name2att=name2att)
	#spectral groups
	...
	if args.balance:
		...
		weight_variable = {}
		bal_weights =  find_class_balanced_wts(train_ids=train_ids, name2att=name2att)
	...
	training_set = Dataset(data_dict=name2feat, name2att=name2att, class_dict=name2class, eszsl_classes=train_classes, list_IDs=train_ids,
	...
	validation_set = Dataset(data_dict=name2feat, name2att=name2att, class_dict=name2class, eszsl_classes=val_classes, list_IDs=val_ids,
							 attributes=predicates, attribute_groups=predicate_groups, groups=groups, adv_dict=adv_dict, zero_shot=args.zero_shot)
	validation_generator = DataLoader(validation_set, **eval_params)

	test_set = Dataset(data_dict=name2feat, name2att=name2att, class_dict=name2class, eszsl_classes=test_classes, list_IDs=test_ids,
					   attributes=predicates, attribute_groups=predicate_groups, groups=groups, adv_dict=adv_dict, zero_shot=args.zero_shot)
...
if __name__ == '__main__':
	...
	parser = argparse.ArgumentParser(description="Grouped Adversarial Learning for Attribute prediction/Zero-Shot learning using images in SUN dataset.")
	...
	parser.add_argument('-cls', '--clusters', help="specify number of spectral grps (>1) - active only when gr_type is sp", default=4, type=int)
