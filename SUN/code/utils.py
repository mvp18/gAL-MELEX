...
def find_class_balanced_wts(train_ids, name2att):

    train_att_mat = np.zeros([len(train_ids), 102])
    for i in range(len(train_ids)):
        train_att_mat[i] = name2att[train_ids[i]]
    attr_count = {}
    for i in range(102):
...
def create_spectral_groups(num_clusters, random_state, predicates, train_ids, test_prior, name2att):

    train_att_mat = np.array([name2att[c] for c in train_ids]).transpose()
