...
from datagen import Dataset, load_data, load_data_train_val_split, load_data_custom_split
from utils import *
...
def main(args):
	...
	# Seed the random states for clustering
	np.random.seed(0)
	...
	if args.data_split == 'gbu':
		...
		all_classes, img_name_list, class_list, signature_list, name2feat, name2class, class2att = load_data()
		ds_type = '' # This is used later while createing folder names
	elif args.data_split == 'tv':
		print('\nLoading data which uses train+val set for training:')
		all_classes, img_name_list, class_list, signature_list, name2feat, name2class, class2att = load_data_train_val_split()
		ds_type = '_train_val'
	elif args.data_split == 'cs':
		new_split_file = '../awa_cs_split.npy'
		print('\Loading data with new splits as in ' + new_split_file)
		all_classes, img_name_list, class_list, signature_list, name2feat, name2class, class2att = load_data_custom_split(new_split_file)
		ds_type = '_corr_shift'


	spectral_groups, attr_distance = create_spectral_groups(num_clusters=args.clusters, random_state=random_state, predicates=predicates,
															all_classes=all_classes, train_classes=class_list[0], test_classes=class_list[2],
															class2att=class2att)
	if args.method_of_grouping=='sp':
		#spectral groups
		predicate_groups = spectral_groups
		print('\nUsing {} attribute groups formed by spectral co-clustering.'.format(len(predicate_groups)))
	elif args.method_of_grouping=='sem':
		#semantic groups
		predicate_groups = np.load('../predicates_groups.npy', allow_pickle=True).item()
	...
	train_ids = img_name_list[0].tolist()
	val_ids = img_name_list[1].tolist()
	test_ids = img_name_list[2].tolist()
	...
	prior_matrix_tr = signature_list[0][:, permute_predicates].transpose()
	prior_matrix_v = signature_list[1][:, permute_predicates].transpose()
	prior_matrix_ts = signature_list[2][:, permute_predicates].transpose()
	...
	# loss_dict with loss functions for predicting multilabel variables
	if args.balance:
		print('\nUsing weighted bce loss')
		weight_variable = {}
		bal_weights =  find_class_balanced_wts(train_ids=train_ids, name2class=name2class, all_classes=all_classes, class2att=class2att)
		...
		if args.zero_shot!=1:
			print('\nLoss for group attr predictions is simple binary cross_entropy')
			for group in groups:
	...
	if loss_weights:
		if args.zero_shot:
			...
			if args.zero_shot==2 or args.zero_shot==3: # For a later time
				for gr in groups:
					loss_weights[gr] = args.predicates_weight
				print('Primary Group Attribute Prediction Loss + Zero-Shot Categorical Loss\n')

		elif args.zero_shot==0:
			for gr in groups:
				loss_weights[gr] = 1.0
		print('Loss weights : {}\n'.format(loss_weights))
	...
	training_set = Dataset(data_dict=name2feat, class2att=class2att, class_dict=name2class, eszsl_classes=train_classes, list_IDs=train_ids,
						   all_classes=all_classes, attributes=predicates, attribute_groups=predicate_groups, groups=groups, adv_dict=adv_dict,
						   zero_shot=args.zero_shot)
	training_generator = DataLoader(training_set, **train_params)

	validation_set = Dataset(data_dict=name2feat, class2att=class2att, class_dict=name2class, eszsl_classes=val_classes, list_IDs=val_ids,
							 all_classes=all_classes, attributes=predicates, attribute_groups=predicate_groups, groups=groups, adv_dict=adv_dict,
							 zero_shot=args.zero_shot)
	validation_generator = DataLoader(validation_set, **eval_params)

	test_set = Dataset(data_dict=name2feat, class2att=class2att, class_dict=name2class, eszsl_classes=test_classes, list_IDs=test_ids,
					   all_classes=all_classes, attributes=predicates, attribute_groups=predicate_groups, groups=groups, adv_dict=adv_dict,
					   zero_shot=args.zero_shot)
	test_generator = DataLoader(test_set, **eval_params)

	if args.baseline or args.grad_reversal_method=='neg-grad':
		print('Gradient Reversal in Operation.\n')
	...
	if args.zero_shot==3: # Not to be used with entropy_cond, wd result in mixup, fix later
		params_to_optimize.append({'params':model.att_pred_eszsl3.parameters()})
	...
	for epoch in range(args.num_epochs):
		...
		elif args.zero_shot==2 or args.zero_shot==3:
			lookup_key='mAP'
			train_disp_key = 'acc'
			train_acc.append(train_score_dict['acc'])
			val_acc.append(val_score_dict[lookup_key])
			test_acc.append(test_score_dict['acc'])
		else:
			lookup_key='mAP'
			train_disp_key = 'mAP'
		...
		print('Train {}:{}; Validation {}:{}; Test {}:{}'.format(train_disp_key, train_score_dict[train_disp_key],\
				lookup_key, val_score_dict[lookup_key], train_disp_key, test_score_dict[train_disp_key]))
		print('\n')

	print('Best performance at epoch {} with Validation {}:{}.\n'.format(bestEp, lookup_key, best_val))

	print('Testing with best model')
	model.load_state_dict(best_model_wts)
	test_score_dict = test_model(model=model, test_generator=test_generator, loss_dict=loss_dict, zero_shot=args.zero_shot,
								 prior_matrix=prior_matrix_ts, device=device)
	print('Test {}:{}'.format(train_disp_key, test_score_dict[train_disp_key]))

if __name__ == '__main__':

	parser = argparse.ArgumentParser(description="Grouped Adversarial Learning for Attribute prediction/Zero-Shot learning using images in AwA dataset.")
	...
	parser.add_argument('-cls', '--clusters', help="specify number of spectral grps (>1) - active only when gr_type is sp", default=6, type=int)
	parser.add_argument('-gr_type', '--method_of_grouping', help="semantic('sem') or spectral('sp')", default='sp', type=str)
	...
	parser.add_argument('-drop', '--drop_rate', help='Dropout rate only for zero shot experiments', default=0.2, type=float)
	...
	parser.add_argument('-pw', '--predicates_weight', help='weight for predicting attributes(predicates) in the case of using ESZSL=2', default=0., type=float)
