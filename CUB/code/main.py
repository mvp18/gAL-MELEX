...
from datagen import Dataset, load_data_custom_split, load_data
from utils import *
...
def main(args):
    ...
    if args.data_split == 'gbu':
        ...
        img_name_list, class_list, signature_list, feature_lookup, attr_labels_dict, class_lookup = load_data()
        ds_type = '' # This is used later while createing folder names
    elif args.data_split == 'cs':
        new_split_file = '../cub_cs_split.npy'
        print('\Loading data with new splits as in ' + new_split_file)
        img_name_list, class_list, signature_list, feature_lookup, attr_labels_dict, class_lookup = load_data_custom_split(new_split_file)
        ds_type = '_corr_shift_'
    ...
    spectral_groups, attr_distance = create_spectral_groups(num_clusters=args.clusters, random_state=random_state, train_img_names=img_name_list[0],
                                                            attr_labels_dict=attr_labels_dict, test_prior=signature_list[2].transpose(),
                                                            predicates=predicates)

    if args.method_of_grouping=='sem':
        predicate_groups = defaultdict(list)
        for x in predicates:
            group = x.strip().split(' ')[1].split('::')[0][4:]
            predicate_groups[group].append(x)
        print('\nUsing semantic groups.')
    elif args.method_of_grouping=='sp':
        #spectral groups
    ...
    def fix_p():
        fixed_p=[]
        for group in groups:
            fixed_p.extend([predicates.index(att) for att in predicate_groups[group]])
        return fixed_p

    # Rearrange the attribute indices of prior matrix according to the new grouping
    prior_matrix_tr = signature_list[0][:, fix_p()].transpose()
    prior_matrix_v = signature_list[1][:, fix_p()].transpose()
    prior_matrix_ts = signature_list[2][:, fix_p()].transpose()
    ...
    if args.balance:
        ...
        weight_variable = {}
        bal_weights =  find_class_balanced_wts(train_img_names=img_name_list[0], attr_labels_dict=attr_labels_dict)
    ...
    if not args.baseline:
        ...
        if args.adv_balance:
            for adv_branch in adv_candidates:
                iou = get_max_weight(predicates, iou_matrix, predicate_groups[adv_branch['parent'][7:]], predicate_groups[adv_branch['group']])
                if iou>=args.adv_threshold:
                    loss_weights_unnorm.append((adv_branch['node_name'], iou))
    ...
    training_set = Dataset(data_dict=feature_lookup, labels_dict=attr_labels_dict, class_dict=class_lookup, eszsl_classes=train_classes, list_IDs=train_ids,
    ...
    validation_set = Dataset(data_dict=feature_lookup, labels_dict=attr_labels_dict, class_dict=class_lookup, eszsl_classes=val_classes, list_IDs=val_ids,
                             attributes=predicates, attribute_groups=predicate_groups, groups=groups, adv_dict=adv_dict, zero_shot=args.zero_shot)
    validation_generator = data.DataLoader(validation_set, **eval_params)

    test_set = Dataset(data_dict=feature_lookup, labels_dict=attr_labels_dict, class_dict=class_lookup, eszsl_classes=test_classes, list_IDs=test_ids,
                       attributes=predicates, attribute_groups=predicate_groups, groups=groups, adv_dict=adv_dict, zero_shot=args.zero_shot)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Grouped Adversarial Learning for Attribute prediction/Zero-Shot learning using images in CUB dataset.")
    ...
    parser.add_argument('-ie', '--increment_epochs', help="epoch no. till which adv_task_weight gets incremented from 0.0 to adv_task_weight, not in effect during baseline", default=10, type=int)
    parser.add_argument('-bs', '--batch_size', help="batch size used for training", default=64, type=int)
    parser.add_argument('-lr', '--learning_rate', help="learning rate for adam/sgd", default=0.01, type=float)
    parser.add_argument('-cls', '--clusters', help="specify number of spectral grps (>1) - active only when gr_type is sp", default=5, type=int)
    ...
    parser.add_argument('-drop', '--drop_rate', help='Dropout rate only for zero shot experiments', default=0.5, type=float)
